...
(function (root) {
  ...
  ResourcesUI.render = function () {
    ...
    const panel = UI.el('<div class="panel">' +
      ...
      'A step assigned to the equipment is scheduled lot by lot, each lot occupying one unit for the attended work plus the process time. Equipment on the <b>24/7</b> calendar keeps running over nights and weekends (curing, burn-in); <b>shop</b> equipment only runs during shifts. ' +
      '<b>Worker pools</b> limit how many people can work at the same time; steps draw their workers from a pool.</p>' +
      '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Name</th><th>Type</th><th class="num">Capacity</th><th class="num">Lot size</th><th class="num">Default process time (h)</th><th>Calendar</th><th>Notes</th><th>Used in</th><th></th></tr></thead><tbody id="res-body"></tbody></table></div></div>');
    ...
    list.slice().sort((a, b) => (a.type === b.type ? 0 : a.type === 'labor' ? 1 : -1) || String(a.name).localeCompare(String(b.name))).forEach(r => {
      ...
      const tr = UI.el('<tr>' +
        ...
        '<td><select ' + (labor ? 'disabled' : '') + '><option value="24_7"' + (r.calendar !== 'shop' ? ' selected' : '') + '>24/7</option><option value="shop"' + (r.calendar === 'shop' ? ' selected' : '') + '>Shop hours</option></select></td>' +
