...
(function (root) {
  ...
  ResourcesUI.render = function () {
    ...
    const panel = UI.el('<div class="panel">' +
      ...
      '<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Name</th><th>Type</th><th class="num" title="Equipment: number of identical units. Worker pool: persons">Capacity</th><th class="num" title="Equipment only: pieces one unit holds per run">Lot size</th><th class="num" title="Equipment only: run time per lot, copied to steps as a default">Default process time (h)</th><th>Calendar / shifts</th><th>Notes</th><th>Used in</th><th></th></tr></thead><tbody id="res-body"></tbody></table></div></div>');
    ...
    list.slice().sort((a, b) => (a.type === b.type ? 0 : a.type === 'labor' ? 1 : -1) || String(a.name).localeCompare(String(b.name))).forEach(r => {
      ...
      const tr = UI.el('<tr>' +
        ...
        (labor ? '<td class="num muted" title="Not used for worker pools">n/a</td><td class="num muted" title="Not used for worker pools">n/a</td>' :
        '<td class="num"><input type="number" min="0" step="1" value="' + U.num(r.lotSize, 1) + '" title="Pieces one unit holds per run (0 = whole batch at once)"></td>' +
        '<td class="num"><input type="number" min="0" step="0.25" value="' + U.num(r.processHours, 0) + '" title="Run time in hours, copied to a step when you pick this resource; the step can override it"></td>') +
      ...
      const inp = c => cells[c].querySelector('input, select');
      UI.bind(inp(0), r, 'name');
      UI.bind(inp(1), r, 'type', 'text', () => ResourcesUI.render());
      UI.bind(inp(2), r, 'capacity', 'int', v => { if (v < 1) { r.capacity = 1; inp(2).value = 1; Store.save(); } });
      if (!labor) { UI.bind(inp(3), r, 'lotSize', 'num'); UI.bind(inp(4), r, 'processHours', 'num'); }
      const f = { 5: inp(5), 6: inp(6) };
