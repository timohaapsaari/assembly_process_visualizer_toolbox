...
(function (root) {
  ...
  PlanUI.renderResult = function (res) {
    ...
    if (res.recipe.expanded) alerts.unshift({ cls: 'info', text: 'Recipe chain: ' + res.recipe.subRecipes.map(x => '"' + x.recipe.name + '" (' + x.code + '.…) makes ' + ((pb[x.partId] || {}).itemNr || '') + (x.viaPlaceholder && x.viaPlaceholder.length ? ' (step ' + x.viaPlaceholder.join(', ') + ' of this recipe has no components, so it runs after that chain as an extra operation on the same item)' : '')).join(', ') + '. Their steps are scheduled as part of this plan.' });
    (res.recipe.unresolved || []).forEach(u => {
      const p = pb[u.partId] || {};
      let why;
      if (!u.makers.length) why = /without components/.test(u.via) ? 'nothing is exploded below it. Add its components to that step, or create a recipe whose final product is this item.' : 'no recipe has a step producing it, so it is treated as a purchased part. Create a recipe whose final product is this item, or set the part type to purchased.';
