...
(function (root) {
  ...
  PlanUI.renderResult = function (res) {
    ...
    if (res.recipe.expanded) alerts.unshift({ cls: 'info', text: 'Recipe chain: ' + res.recipe.subRecipes.map(x => '"' + x.recipe.name + '" (' + x.code + '.…) makes ' + ((pb[x.partId] || {}).itemNr || '') + (x.viaPlaceholder && x.viaPlaceholder.length ? ' (linked from step ' + x.viaPlaceholder.join(', ') + ' of this recipe, whose own times are ignored)' : '')).join(', ') + '. Their steps are scheduled as part of this plan.' });
